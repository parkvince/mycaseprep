...
import { Users, Search, ShieldCheck, Ban, MessageSquare, Activity } from "lucide-react";
...
export default function AdminPage() {
  ...
  const [view, setView] = useState<"users" | "feedback" | "activity">("users");
    ...
    <div style={{ minHeight: "100vh", background: "oklch(0.985 0.005 285)", fontFamily: FONT }}>
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "2.5rem 2rem 5rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem" }}>
          {([["users", "Users"], ["feedback", "Feedback"], ["activity", "Activity"]] as const).map(([v, label]) => (
            <button
              key={v}
              onClick={() => setView(v)}
            ...
            >
              {v === "users" ? <Users size={15} /> : v === "feedback" ? <MessageSquare size={15} /> : <Activity size={15} />} {label}
            </button>
          ))}
        </div>
